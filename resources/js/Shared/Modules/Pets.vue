<template>
  <div class="mb-10">
    <!-- title + cta -->
    <div class="mb-3 items-center justify-between border-b border-border pb-2 dark:border-border flex">
      <div class="mb-2 sm:mb-0 flex items-center gap-2">
        <Dog class="h-4 w-4 text-text" />

        <span class="crm-title"> {{ $t('Pets') }} </span>
      </div>
        ...
        <li
          ...
          <div v-if="editedPetId !== pet.id" class="flex items-center justify-between px-3 py-2">
            <div class="flex items-center">
              <span class="me-2 text-sm text-text-muted">{{ pet.pet_category.name }}</span>
          ...
          <!-- edit pet modal -->
          <form v-if="editedPetId === pet.id" class="bg-bg dark:bg-surface" @submit.prevent="update(pet)">
            <div class="border-b border-border dark:border-border">
              <div v-if="form.errors.length > 0" class="p-5">
                <errors :errors="form.errors" />
              </div>
