  ...
  <div class="mb-10">
    ...
    <div class="crm-panel-header justify-between">
      <div class="mb-2 sm:mb-0 flex items-center gap-2">
        <UsersRound class="h-4 w-4 text-text" />

        <span class="crm-title"> {{ $t('Relationships') }} </span>
      </div>
    ...
    <div>
      <div v-for="relationshipGroupType in localRelationships" :key="relationshipGroupType.id" class="mb-4">
        ...
        <ul
          ...
          class="mb-4 rounded-lg border border-border last:mb-0 dark:border-border">
          <li
            ...
            <div class="flex">
              <div class="me-2 flex items-center">
                ...
                <!-- age -->
                <span v-if="relationshipType.contact.age" class="ms-2 text-xs text-text-muted"
                  >({{ relationshipType.contact.age }})</span
                >
              </div>

              <!-- relationship type -->
              <span class="me-2 text-text-muted">{{ relationshipType.relationship_type.name }}</span>
            </div>
