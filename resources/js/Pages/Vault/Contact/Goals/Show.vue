      ...
      <div class="mx-auto max-w-5xl px-2 py-2 sm:px-6 sm:py-6 lg:px-8">
        <div class="special-grid grid grid-cols-1 gap-6 sm:grid-cols-3">
          ...
          <div class="p-3 sm:px-3 sm:py-0">
            ...
            <form
              v-if="editMode"
              class="mb-4 rounded-lg border border-border bg-bg dark:border-border dark:bg-surface"
            ...
            <!-- stats -->
            <div class="mb-6 flex justify-between rounded-xs border border-border p-3 dark:border-border">
              <div class="me-6 flex items-center">
                <div class="me-3 w-14 text-right text-sm text-text-muted">{{ $t('Total streaks') }}</div>
                <div class="text-4xl">
                  {{ localGoal.count }}
                </div>
              </div>
              ...
              <div v-for="week in localGoal.weeks" :key="week.id">
                <div v-for="streak in week.streaks" :key="streak.id">
                  <a-tooltip placement="topLeft" :title="streak.date" arrow-point-at-center>
                    ...
                    <div
                      v-if="!streak.not_yet_happened && !streak.streak"
                      class="h-3 w-3 cursor-pointer rounded-xs bg-bg"></div>
                  </a-tooltip>
                </div>
              </div>
            </div>
